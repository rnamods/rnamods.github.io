...
fetch(fn)
  ...
  .then( t => {
	let outtext = t;
    outtext = outtext.replace("|COMPO|", mycompo.replace(/(\d+)/g,"<SUB>$1</SUB>"));
...
thetxt = thetxt.replace(/doi\:([\w\/\.\_\-\;\(\)]+)([\s\n])/g,"<A HREF='https://doi.org/$1'>doi:$1</A>$2");	
...
document.getElementById("numresults").innerText = thesele.length + " records found.";
// show table
doMenu("outtable","");
}
//------------------------------------------
function loadInit() {
var fn;
fn = baseurl + "txt/rnabtoa.txt"; // load database
fetch(fn)
  .then( r => r.text() )
  .then( t => fillData(atob(t)) );
fn = baseurl + "txt/rnasear.txt"; // load search page
fetch(fn)
  .then( r => r.text() )
  .then( t => {document.getElementById('input').innerHTML = t;} );
